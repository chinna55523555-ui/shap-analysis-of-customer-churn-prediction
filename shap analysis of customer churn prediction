...
# 8. SHAP analysis
# Ensure X_train is purely numerical (float) before passing to SHAP explainer
X_train_numeric = X_train.astype(float)
explainer = shap.Explainer(best, X_train_numeric)   # TreeExplainer under the hood for tree models
shap_values = explainer(X_test.astype(float), check_additivity=False)
...
for i, idx in enumerate(selected_indices):
    ...
    shap.plots._waterfall.waterfall_legacy(sv.base_values, sv.values, features=sv.data, max_display=8, show=False)
...
1. Target month-to-month customers with rete
